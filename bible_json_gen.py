    ...
    for ch in chapters:
        ...
        for v in verses:
            ...
            for w_match in re.finditer(
                r'\\w\s+([^\|]+)\|lemma="([^"]+)"\s+strong="([^"]+)"\s+x-morph="([^"]+)"\s+x-id="([^"]+)"\\w\*',
                v
            ):
                word_text, lemma, strong, x_morph, x_id = w_match.groups()
                words.append({
                    "text": word_text,
                    "lemma": lemma,
                    "strong": strong,
...
# ------------------------------
# input_folder = "./usfm/uhb"   # dossier contenant les fichiers .usfm
# output_folder = "./json/uhb"  # dossier où seront enregistrés les JSON
# ------------------------------
